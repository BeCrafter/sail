...
- **服务端固定延迟会按对象数放大**:某些自建 S3 兼容网关的单次 DELETE 固定 ~27-30s(连不存在的 key 也一样),批量 `DeleteObjects` 返 500 不可用;标准云对象存储对照 38ms 正常。**解决**:sail 侧已由 `internal/s3del` 并发化缓解,**总耗时 ≈ ⌈对象数/并发度⌉ × 单次延迟**;网关侧属其自身缺陷,最小复现 = 对不存在的 key 发一次 DELETE 并计时(或观察批量端点返 500),应在网关侧修复,不要在 sail 侧再加特例适配。
