...
`--max-upload-size` 与配额同在提交点判定,因此也带上了同一条限制:对象不会落库,但客户端不会被告知——见下方差异表。
...
配额由与 WebDAV 模式同一个协议无关装饰器执行:提交点判定,超限的写入被拒绝且不落库。不同的
是客户端看到什么 —— 见下方「SMB 特有的限制」里那条静默拒绝。
...
| 配额/超限错误 | `507 Insufficient Storage` / `413` | 提交点拒绝,但客户端不会被告知(见下) |
...
- **超配额/超限的写入会被拒绝,但客户端不会被告知**。判定只能放在提交点(SMB2 不预先携带
  Content-Length),而提交发生在客户端关闭句柄时,库又忽略它的返回值 —— 于是对象确实没有被写
  (超配额的数据一个字节都不会落库),客户端却看到一次成功的关闭,只能事后自己发现;服务端日志里
  有对应那一行。另外库自己把文件系统错误翻译成 NTSTATUS、没有留映射钩子,所以哪怕错误真的传到了
  客户端,`STATUS_DISK_FULL` 与 `STATUS_FILE_TOO_LARGE` 也取不到。
...
- **目录列表有缓存**(`--dir-cache-ttl`,默认 60s),自己的写入会立刻失效自己的目录;别人在别处
  改动的可见延迟最长一个 TTL。
- **改名一个仍被打开的文件,内容会写回旧名字**。壳的句柄绑定在打开时的路径上,库的改名只更新
  它自己那份记录,于是关闭时的提交落到改名前那条路径:旧名字带着新内容重新出现,改名后的文件
  仍是旧字节。文件管理器正常都是先关闭再改名,所以这很少见 —— 但壳内没法察觉。
