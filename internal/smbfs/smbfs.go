...
func shareNames(cfg Config) ([]Share, error) {
	...
	for _, u := range cfg.Users {
		...
		if prev, dup := seen[strings.ToUpper(name)]; dup {
			return nil, fmt.Errorf("smbfs: users %q and %q both map to share %q; share names must differ", prev, u.Name, name)
		}
...
func validateShareName(name string, fromUserName bool) error {
	...
	switch {
	case name == "":
		return fmt.Errorf("%s must not be empty", origin)
	case strings.ContainsAny(name, `\/`):
		return fmt.Errorf("%s contains a path separator, which SMB does not allow in a share name (%q)", origin, name)
	case strings.EqualFold(name, "IPC$"):
		return fmt.Errorf("%s is the SMB protocol's own IPC$ name (%q)", origin, name)
	case strings.Contains(name, ":"):
		return fmt.Errorf("%s contains \":\", which clients read as a port or stream separator (%q)", origin, name)
	}
	return nil
}
