...
func runServeSmb(cmd *cobra.Command, _ []string) error {
	...
	if err != nil {
		return translateSMBStartError(err)
	}
	defer srv.Close()
...
// translateSMBStartError keeps smbfs independent from the CLI's i18n package
// while ensuring startup validation errors obey --lang at the command edge.
func translateSMBStartError(err error) error {
	var shareErr *smbfs.ShareNameError
	if errors.As(err, &shareErr) {
		if shareErr.FromUserName {
			switch shareErr.Reason {
			case smbfs.ShareNameEmpty:
				return errors.New(i18n.Tf("SMB user name %q must not be empty", shareErr.Name))
			case smbfs.ShareNamePathSeparator:
				return errors.New(i18n.Tf("SMB user name %q (the share name in multi-user mode) contains a path separator, which SMB does not allow", shareErr.Name))
			case smbfs.ShareNameReserved:
				return errors.New(i18n.Tf("SMB user name %q (the share name in multi-user mode) is reserved by the SMB protocol (IPC$)", shareErr.Name))
			case smbfs.ShareNameColon:
				return errors.New(i18n.Tf("SMB user name %q (the share name in multi-user mode) contains \":\", which clients may read as a port or stream separator", shareErr.Name))
			default:
				return errors.New(i18n.Tf("invalid SMB user name %q", shareErr.Name))
			}
		}
		switch shareErr.Reason {
		case smbfs.ShareNameEmpty:
			return errors.New(i18n.Tf("SMB share name %q must not be empty", shareErr.Name))
		case smbfs.ShareNamePathSeparator:
			return errors.New(i18n.Tf("SMB share name %q contains a path separator, which SMB does not allow", shareErr.Name))
		case smbfs.ShareNameReserved:
			return errors.New(i18n.Tf("SMB share name %q is reserved by the SMB protocol (IPC$)", shareErr.Name))
		case smbfs.ShareNameColon:
			return errors.New(i18n.Tf("SMB share name %q contains \":\", which clients may read as a port or stream separator", shareErr.Name))
		default:
			return errors.New(i18n.Tf("invalid SMB share name %q", shareErr.Name))
		}
	}
	var duplicateErr *smbfs.DuplicateShareError
	if errors.As(err, &duplicateErr) {
		return errors.New(i18n.Tf(
			"SMB users %q and %q resolve to the same share %q; share names must differ",
			duplicateErr.PreviousUser, duplicateErr.User, duplicateErr.Name))
	}
	return err
}
...
func init() {
	...
	serveSmbCmd.Flags().StringVar(&serveSmbOpts.maxUploadSize, "max-upload-size", "", "per-file size limit, defaults to --backend-max-object-size; over the limit nothing is written, but the client is not told (the check runs at close, and the library ignores that result — see the command help)")
